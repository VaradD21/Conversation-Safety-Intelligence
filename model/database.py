...
def get_connection() -> sqlite3.Connection:
    ...
    # Enable WAL mode for safe concurrent reads during FastAPI worker threads
...
def init_db() -> None:
    with get_connection() as conn:
        ...
        c.execute('CREATE INDEX IF NOT EXISTS idx_interactions_user_id ON interactions(user_id)')
        c.execute('CREATE INDEX IF NOT EXISTS idx_interactions_conversation_id ON interactions(conversation_id)')
        conn.commit()
...
def get_user(user_id: str) -> Dict[str, object]:
    with get_connection() as conn:
        c = conn.cursor()
        c.execute(
            'SELECT user_id, risk_score, total_flags, last_updated, grooming_stage_max FROM users WHERE user_id = ?',
            (user_id,),
        )
        row = c.fetchone()
        if not row:
            ...
            c.execute(
                'INSERT INTO users (user_id, risk_score, total_flags, last_updated, grooming_stage_max) VALUES (?, ?, ?, ?, ?)',
...
def update_user_risk(user_id: str, risk_level: str) -> None:
    score_increment = 0
    flag_increment = 0
...
def get_user_interaction_stats(user_id: str) -> Dict[str, int]:
    with get_connection() as conn:
        c = conn.cursor()
        c.execute(
            'SELECT COUNT(*), COUNT(DISTINCT conversation_id) FROM interactions WHERE user_id = ?',
            (user_id,),
        )
        total_inter, unique_convos = c.fetchone()

        c.execute(
            'SELECT COUNT(*) FROM interactions WHERE user_id = ? AND risk_level = "hazardous"',
            (user_id,),
        )
